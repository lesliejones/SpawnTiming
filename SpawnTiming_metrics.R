...
names(spawn) <- tolower(names(spawn))
spawn$date <- as.Date(spawn$date, format = "%m/%d/%Y")
spawn$site.year <- paste(spawn$site, spawn$year, sep = "_")
spawn$mean <- spawn$temperature
spawn$max <- rnorm(dim(spawn)[1], mean = 7, sd = 1)
spawn$min <- rnorm(dim(spawn)[1], mean = 2, sd = 0.5)

facets <- tempmetrics(spawn[complete.cases(spawn),], "spawn_facets")
